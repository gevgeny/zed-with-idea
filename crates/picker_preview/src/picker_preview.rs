...
pub fn editor_preview(
    ...
    Arc::new(EditorPreviewHandle(cx.new(|cx| {
        EditorPreview::new(project, Interactivity::Inert, window, cx)
    })))
}
...
/// a multibuffer. A preview the user reads rather than clicks through keeps its own scrolling,
/// its scrollbar, and the mouse events needed to select text.
pub fn scrollable_editor_preview(
    ...
    Arc::new(EditorPreviewHandle(cx.new(|cx| {
        EditorPreview::new(project, Interactivity::Scrollable, window, cx)
    })))
}
...
impl EditorPreview {
    ...
    fn scroll_to_focus_match(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        // Scrolling to the match needs the lock off; put it back only for previews meant to be
        // inert, or a scrollable one is re-locked on every update.
